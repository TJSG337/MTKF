...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None, kpt_label=False, step=2):
    # Rescale coords (xyxy) from img1_shape(old) to img0_shape(new)  e.g.old=640,640;new=474,650。640/650=0.9846
...
def clip_coords(boxes, img_shape, step=2):
    ...
    boxes[:, 0::step].clamp_(0, img_shape[1])  # x1 step为2的话，即为隔一个取一次。取0,2
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), kpt_label=False, nc=None, nkpt=None, need_overlap_test=False):
    ...
    if nc is None:
        ...
        nc = prediction.shape[2] - 5 if not kpt_label else prediction.shape[2] - my_kpt * 3 - 5
    ...
    t = time.time()
    output = [torch.zeros((0, 7 + my_kpt * 3), device=prediction.device)] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference此时的x是10=xywh+obj+cls1+cls2+xy+clsk
        # Apply constraints
        ...
        # flag = multi_label
        flag = False
        if flag:  # multi_label会导致同一个目标出现多个iou重叠很大的bbox。
            kpts = x[:, 7:]
            # i, j = (x[:, 5:] > conf_thres).nonzero(as_tuple=False).T
            i, j = (x[:, 5:-my_kpt * 3] > conf_thres).nonzero(as_tuple=False).T
            ...
            if not kpt_label:
                ...
                kpts = x[:, 7:]  # 两类，所以从7开始是关键点
                # conf, j = x[:, 5:6].max(1, keepdim=True)
                conf, j = x[:, 5:-my_kpt * 3].max(1, keepdim=True)
        ...
        if need_overlap_test:

            iou_threshold = 0.8
            output_test = x[i]
            output[xi] = filter_high_iou_bboxes(output_test, iou_threshold)
