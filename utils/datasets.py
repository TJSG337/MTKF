                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        ...
                        if kpt_label:
                            ...
                            assert l.shape[1] == (1 + 4 + self.my_kpt * 3), 'labels require 5+kpts*3 columns each'
                            ...
                            for i in range(len(l)):
                                kpt = np.delete(l[i, 5:], np.arange(2, l.shape[1] - 5,
                                                                    3))  # remove the occlusion paramater from the GT
                                kpts[i] = np.hstack((l[i, :5], kpt))
                            l = kpts

                            assert l.shape[
                                       1] == 1 + 4 + self.my_kpt * 2, 'labels require 39 columns each after removing occlusion paramater'
                        ...
                        l = np.zeros((0, 1 + 4 + self.my_kpt * 2), dtype=np.float32) if kpt_label else np.zeros((0, 5),
                                                                                                                dtype=np.float32)
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if labels.size:
            labels[:, 1:] = xywhn2xyxy(labels[:, 1:], w, h, padw, padh, kpt_label=self.kpt_label)
    ...
    if n:
        ...
        if use_segments:  # warp segments
            ...
            if kpt_label:
                ...
                xy_kpts = np.ones((n * my_kpt, 3))
                # xy_kpts[:, :2] = targets[:, 5:].reshape(n * 17, 2)  # num_kpt is hardcoded to 17
                xy_kpts[:, :2] = targets[:, 5:].reshape(n * my_kpt, 2)  # num_kpt is hardcoded to 17
                ...
                xy_kpts = (xy_kpts[:, :2] / xy_kpts[:, 2:3] if perspective else xy_kpts[:, :2]).reshape(n, my_kpt * 2)
                xy_kpts[targets[:, 5:] == 0] = 0  # 让targets里面，本来是0的关键点等于0
                # x_kpts = xy_kpts[:, list(range(0, 34, 2))]
                # y_kpts = xy_kpts[:, list(range(1, 34, 2))]
                x_kpts = xy_kpts[:, list(range(0, my_kpt * 2, 2))]
                y_kpts = xy_kpts[:, list(range(1, my_kpt * 2, 2))]
                ...
                xy_kpts[:, list(range(0, my_kpt * 2, 2))] = x_kpts
                xy_kpts[:, list(range(1, my_kpt * 2, 2))] = y_kpts
