...
def plot_skeleton_kpts(im, kpts, steps, orig_shape=None):
    ...
    # pose_limb_color = palette[[9, 9, 9, 9, 7, 7, 7, 0, 0, 0, 0, 0, 16, 16, 16, 16, 16, 16, 16]]  # 对应不同骨骼连接线的颜色
    pose_limb_color = palette[[9, 9, 9]]
    # pose_kpt_color = palette[[16, 16, 16, 16, 16, 0, 0, 0, 0, 0, 0, 9, 9, 9, 9, 9, 9]]  # 对应不同关键点的颜色
    pose_kpt_color = palette[[16]]
    ...
    for kid in range(num_kpts):
        ...
        x_coord, y_coord = kpts[steps * kid], kpts[steps * kid + 1]  # 获取一个关键点的坐标
...
def output_to_target(output):  # nc无论为多少,单个output维数都为9,即为xyxy+cls得分+cls标签+xy+clskp!!!
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16,
    ...
    # Plot image grid with labels 此时的target单个维数是9,bs+cls号+xywh++cls分+xy
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            labels = image_targets.shape[1] == 8 if kpt_label else image_targets.shape[1] == 6
