...
class ComputeLoss:
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.kpt_label:
                    ...
                    # mask
                    kpt_mask = (tkpt[i][:, 0::2] != 0)  # 切的都是每个关键点的x
                    ...
                    #         (1 - torch.exp(-d / (s * (4 * sigmas ** 2) + 1e-9))) * kpt_mask).mean()  # 这个就是oks
                    kpt_loss_factor = (torch.sum(kpt_mask != 0) + torch.sum(kpt_mask == 0)) / (
                                torch.sum(kpt_mask != 0) + 0.01)  # v0.1
                    # # kpt_loss_factor = (torch.sum(kpt_mask != 0) + torch.sum(kpt_mask == 0)) / torch.sum(kpt_mask !=
                    # # 0) 尝试修复关键点飘的问题 v0.0
                    lkpt += kpt_loss_factor * (((1 - torch.exp(
                        -d / (2 * (s * sigmas) ** 2 + 1e-9))) + 0.05 * d) * kpt_mask).mean()  # v0.1
    ...
    # target(333,40)--icxywh+17*2(6+34)
    def build_targets(self, p, targets):
        ...
        if self.kpt_label:
            ...
            gain = torch.ones(7 + self.my_kpts * 2, device=targets.device)
        else:
        ...
        for i in range(self.nl):
            ...
            if self.kpt_label:
                ...
                gain[2:6 + self.my_kpts * 2] = torch.tensor(p[i].shape)[(2 + self.my_kpts) * [3, 2]]
            else:
                gain[2:6] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain

            # Match targets to anchors
            t = targets * gain  # tgt--bs,c,xywh+xy+a=9
            if nt:
                # Matches
            ...
            if self.kpt_label:
                for kpt in range(self.nkpt):  # 此时t(icxywh+34+a=41)该操作是将对应不等于0的关键点坐标减对应的gij！
                    # 这是标签,一个点的为icxywh+xy+a=9
                    t[:, 6 + 2 * kpt: 6 + 2 * (kpt + 1)][t[:, 6 + 2 * kpt: 6 + 2 * (kpt + 1)] != 0] -= gij[
                        t[:, 6 + 2 * kpt: 6 + 2 * (kpt + 1)] != 0]
                tkpt.append(t[:, 6:-1])
            anch.append(anchors[a])  # anchors 将model里面yaml的anchor尺寸除以对应的stride,表示fmap上的尺寸
            tcls.append(c)  # class
