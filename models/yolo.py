...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), nkpt=None, ch=(), inplace=True, dw_conv_kpt=False):  # detection layer
        ...
        #                                   DWConv(x, x, k=3), Conv(x, x),
        #                                   DWConv(x, x, k=3), Conv(x, x)) for x in ch)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            if self.nkpt is None or self.nkpt == 0:
                ...
                # x[i] = self.middle[i](x[i])
                x[i] = torch.cat((self.m[i](x[i]), self.m_kpt[i](x[i])), axis=1)

            bs, _, ny, nx = x[i].shape  # x(bs,171,32,32) to x(bs,3,32,32,57)这里是将det与kpt用两个不同的头检测，然后再cat起来
            x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()

            x_det = x[i][..., :5+self.nc]  # det--xywh+o+c1+c2
            x_kpt = x[i][..., 5+self.nc:]
            ...
            if not self.training:  # inference
                ...
                if self.inplace:
                    ...
                    wh = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i].view(1, self.na, 1, 1, 2)  # wh self.anchor_grid是配置文件里面的anchor尺寸
                    if self.nkpt != 0:
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, ConvFocus, CrossConv,
                 BottleneckCSP,
                 C3, C3TR, Bottleneck_sa, C3_sa, C3_LSK, C3_Clo, C3MA]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, Bottleneck_sa, C3_sa, C3_LSK, C3_Clo, C3MA]:
                args.insert(2, n)  # number of repeats
                n = 1
            if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, DWConv, MixConv2d, Focus, ConvFocus, CrossConv,
                     BottleneckCSP, C3, C3TR, Bottleneck_sa, C3_sa, C3_LSK, C3_Clo, C3MA]:
                if 'act' in d.keys():
                    args_dict = {"act": d['act']}
