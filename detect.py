...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        print(pred[..., 4].max())  # 打印出来obj最高的数
        # Apply NMS
        pred = non_max_suppression(pred, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms,
                                   kpt_label=kpt_label, need_overlap_test=True)  # 出来的是xyxy+cls分+cls号+xy+cls3,维数为9
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(reversed(det[:, :6])):
                    if save_txt:  # Write to file
                        if save_xyxy:
                            ...
                            line = (cls, *xyxy, conf, *kpts) if opt.save_conf else (cls, *xyxy, *kpts)  # label format
                        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-xyxy', action='store_true', help='use xyxy coordinations')
