...
from utils.general import coco80_to_coco91_class, check_dataset, check_file, check_img_size, \
    check_requirements, box_iou, non_max_suppression, scale_coords, xyxy2xywh, xywh2xyxy, set_logging, increment_path, \
    colorstr
...
def filter_pred_with_keypoints(pred):
    filtered_pred = []
    ...
    for bbox in pred:
        ...
        keypoint = bbox[6:]
        if keypoint[2] < 0.2:
            continue  # 跳过当前bbox，继续处理下一个bbox

        if (keypoint[0] >= bbox_corners[0]) and (keypoint[0] <= bbox_corners[2]) and (
                keypoint[1] >= bbox_corners[1]) and (keypoint[1] <= bbox_corners[3]):
            filtered_pred.append(bbox)

    if not filtered_pred:  # 判断是否为空列表
        return torch.empty((0, pred.shape[1]), dtype=pred.dtype, device=pred.device)  # 返回空张量

    return torch.stack(filtered_pred)
...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            out, train_out = model(img, augment=augment)  # inference and training outputs 这里的outputs输出的是10没错
            if flip_test:
            ...
            out = out[..., :7] if not kpt_label else out  # 这里两类1点，前7个数对应的是xywh+obj+cls1+cls2
            ...
            # Run NMS
            if kpt_label:  # 在此，tgt转换为全局像素坐标
                num_points = (targets.shape[1] // 2 - 1)
                targets[:, 2:] *= torch.Tensor([width, height] * num_points).to(device)  # 从归一化转为全局像素化tgt:icxywh+xy
            else:
            ...
            out = non_max_suppression(out, conf_thres, iou_thres, labels=lb, multi_label=False, agnostic=single_cls,
                                      ...
                                      need_overlap_test=False)

            # out = non_max_suppression_src(out, conf_thres=conf_thres, iou_thres=iou_thres)
        ...
        for si, pred in enumerate(out):
            ...
            labels = targets[targets[:, 0] == si, 1:]  # 提取该si张图片的标签,labels为cxywh+xy=7
            nl = len(labels)
            ...
            # 此时的predn维数为9,为xyxy+c得分+cls+xy+clskp
            ...
            if nl:
                ...
                # target boxes
                tbox = xywh2xyxy(labels[:, 1:5])  # 此时仍然是640的,labels为7，cxywh+xy
                scale_coords(img[si].shape[1:], tbox, shapes[si][0], shapes[si][1],
                             kpt_label=False)  # 将640的box坐标转移到真实图片的尺寸上。native-space labels, kpt_label is set to False as we are dealing with boxes here
                if kpt_label:
                    tkpt = labels[:, 5:]  # 此时的是640*640的
                    scale_coords(img[si].shape[1:], tkpt, shapes[si][0], shapes[si][1],
                                 kpt_label=kpt_label)  # 将640的坐标转移到真实图片的尺寸上。native-space labels
                    # ！！！此时，tbox和tkpt是真实图片的全局像素坐标！！！
                if plots:
